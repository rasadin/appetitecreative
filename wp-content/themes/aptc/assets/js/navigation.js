...
(function($) {
    ...
    var aptcMenu = $('.aptc-inline-menu');
    $('.aptc-inline-menu li').find('> .sub-menu').before('<span class="mb-toggle js-show-mb-nav"><i class="fas fa-chevron-down"></i></span>'); // Desktop Menu

    var desktopMenu = function desktopMenu() {
        if ($(window).width() > screenWidth) {
            var aptcMenuLink = $('.aptc-inline-menu li');
            aptcMenuLink.find('> ul.sub-menu').addClass('hide');
            aptcMenuLink.on('mouseover', function() {
                $(this).find('> ul.sub-menu').addClass('open').removeClass('hide').stop(true, true);
            });
            aptcMenuLink.on('mouseleave', function() {
